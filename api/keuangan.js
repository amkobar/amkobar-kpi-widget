...
module.exports = async (req,res) => {
  ...
  try {
    ...
    const aktifRows=aktif.length===0?`<tr><td colspan="5" class="empty">Tidak ada project aktif</td></tr>`:aktif.map(o=>`<tr><td><div style="font-weight:500">${o.nama}</div><div style="font-size:10px;color:#475569">${o.nim}</div></td><td>${o.layanan}</td><td style="color:#475569">${o.aplikasi}</td><td>${statusBadge(o.status,o.isLate)}</td><td class="${o.isLate?'red':o.diff!==null&&o.diff<=3?'amber':'m'}">${fmtDate(o.deadline)}</td></tr>`).join('');

    const urgentList=[...terlambat,...deadlineDekat];
    const urgentRows=urgentList.length===0?`<tr><td colspan="4" class="empty">Tidak ada deadline mendesak</td></tr>`:urgentList.map(o=>{const label=o.diff<0?`Terlambat ${Math.abs(o.diff)} hari`:o.diff===0?'Hari ini':`${o.diff} hari lagi`;return`<tr><td>${o.nama}</td><td>${o.layanan}</td><td class="${o.diff<0?'red':'amber'}">${fmtDate(o.deadline)}</td><td class="${o.diff<0?'red b':'amber b'}">${label}</td></tr>`;}).join('');
...
.val.red{color:#f87171}.val.amber{color:#fbbf24}
...
.red{color:#f87171}.amber{color:#fbbf24}.m{color:#475569}.b{font-weight:600}
.empty{text-align:center;color:#475569;padding:20px}
@media(max-width:600px){.grid{grid-template-columns:repeat(2,1fr);gap:12px}.grid .card:nth-child(3){grid-column:span 2}.val{font-size:26px}}
@media(max-width:380px){.grid{grid-template-columns:1fr}.grid .card:nth-child(3){grid-column:span 1}}
</style></head><body>
${alertBanner}
<div class="grid">
  <div class="card"><div class="lbl">Project Aktif</div><div class="val">${aktif.length}</div><div class="sub">Sedang berjalan</div></div>
  <div class="card"><div class="lbl">Deadline Minggu Ini</div><div class="val amber">${deadlineDekat.length}</div><div class="sub">Perlu perhatian</div></div>
  <div class="card"><div class="lbl">Project Terlambat</div><div class="val red">${terlambat.length}</div><div class="sub">Segera tindak lanjut</div></div>
</div>
<div class="sec"><div class="shdr"><div class="stitle">Semua project aktif</div><div class="smeta">${aktif.length} project</div></div><div style="overflow-x:auto"><table><thead><tr><th>Nama client</th><th>Layanan</th><th>Aplikasi</th><th>Status</th><th>Deadline</th></tr></thead><tbody>${aktifRows}</tbody></table></div></div>
<div class="sec"><div class="shdr"><div class="stitle">Deadline mendesak</div><div class="smeta">Terlambat & 7 hari ke depan</div></div><table><thead><tr><th>Nama client</th><th>Layanan</th><th>Deadline</th><th>Keterangan</th></tr></thead><tbody>${urgentRows}</tbody></table></div>
<div class="sec"><div class="shdr"><div class="stitle">Distribusi status project</div><div class="smeta">Aktif saja</div></div><div class="cw"><canvas id="cS"></canvas></div></div>
<script>
new Chart(document.getElementById('cS'),{type:'bar',data:{labels:${sLabels},datasets:[{data:${sData},backgroundColor:['#64748b','#3b82f6','#10b981','#f59e0b','#8b5cf6'],borderRadius:6,borderSkipped:false}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{display:false},ticks:{color:'#475569',font:{size:11},stepSize:1}},y:{grid:{display:false},ticks:{color:'#475569',font:{size:11}}}}}});
<\/script></body></html>`;

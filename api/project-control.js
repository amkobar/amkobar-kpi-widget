...
gR = function(){
  oldgR();
  var p=document.getElementById('prev-review');
  if(p && R.review) p.textContent=decodeText(R.review);
}

// PATCH gM
var oldgM = gM;
gM = function(tab,nama){
  oldgM(tab,nama);
  var p=document.getElementById('prev-'+tab);
  if(p && R[tab]) p.textContent=decodeText(R[tab]);
}

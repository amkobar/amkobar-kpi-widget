module.exports = async function handler(req, res) {
  ...
  var filtered = C.filter(function(c){
    ...
    var status = (c.status || '').toLowerCase().trim();

    if(t === 'antrian') return status.includes('antrian');
    if(t === 'pelunasan') return status.includes('pelunasan');
    if(t === 'pendampingan') return status.includes('pendampingan');
    if(t === 'selesai') return status.includes('selesai');
